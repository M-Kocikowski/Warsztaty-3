...
public class SolutionsDAO {
    ...
    private static final String SELECT_LAST_SOLUTIONS = "SELECT * FROM solutions ORDER BY created DESC LIMIT ?";
    ...
    public List<Solution> findLastSolutions(int limit) {

        try (Connection connection = DbUtil.getConnection()) {

            List<Solution> lastSolutions = new ArrayList<>();
            PreparedStatement preparedStatement = connection.prepareStatement(SELECT_LAST_SOLUTIONS);
